 ...
 */
 (function(jQuery) {
	...
	var methods = {
		...
		validate: function() {
			...
			if (element.is("form") || element.hasClass("validationEngineContainer")) {
				...
				var form = element.closest('form, .validationEngineContainer'),
					options = (form.data('jqv')) ? form.data('jqv') : jQuery.validationEngine.defaults,
		...
		*/
		updatePromptsPosition: function(event) {
			...
			if (event && this == window) {
				...
				var form = jQuery(this.closest('form, .validationEngineContainer'));
		...
		*/
		hide: function() {
			 ...
			 if(jQuery(this).is("form") || jQuery(this).hasClass("validationEngineContainer")) {
			 ...
			 jQuery('.'+closingtag).fadeTo(fadeDuration, 0.3, function() {
				 jQuery(this).parent('.formErrorOuter').remove();
				 jQuery(this).remove();
			 });
			 return this;
		 ...
		 */
		 hideAll: function() {
			 ...
			 jQuery('.formError').fadeTo(duration, 300, function() {
				 jQuery(this).parent('.formErrorOuter').remove();
				 jQuery(this).remove();
			 });
			 return this;
		...
		*/
		_onSubmitEvent: function() {
			...
			if (form.data("jqv_submitButton")){
				var submitButton = jQuery("#" + form.data("jqv_submitButton"));
				if (submitButton){
		...
		*/
		_validateFields: function(form) {
			...
			form.find('['+options.validateAttribute+'*=validate]').not(":disabled").each( function() {
				var field = jQuery(this);
				var names = [];
				if (jQuery.inArray(field.attr('name'), names) < 0) {
			...
			if (errorFound) {
				if (options.scroll) {
					...
					if (options.isOverflown) {
						...
						jQuery("html, body").animate({scrollLeft: fixleft},1100)
					}
		...
		*/
		_validateField: function(field, options, skipAjaxValidation) {
			if (!field.attr("id")) {
				field.attr("id", "form-validation-field-" + jQuery.validationEngine.fieldIdCounter);
				++jQuery.validationEngine.fieldIdCounter;
			}
           ...
           if (!options.validateNonVisibleFields && (field.is(":hidden") && !options.prettySelect || field.parent().is(":hidden")))
			...
			for (var i = 0, field_errors = 0; i < rules.length; i++) {
				...
				if (limitErrors && field_errors >= options.maxErrorsPerField) {
					...
					if (!required) {
						var have_required = jQuery.inArray('required', rules);
			...
			if ((fieldType == "radio" || fieldType == "checkbox") && form.find("input[name='" + fieldName + "']").size() > 1) {
				if(positionType === 'inline') {
					field = jQuery(form.find("input[name='" + fieldName + "'][type!=hidden]:last"));
				} else {
		...
		*/
		_handleStatusCssClasses: function(field, options) {
		 ...
		 _getErrorMessage:function (form, field, rule, rules, i, options, originalValidationMethod) {
			 ...
			 if (errorMsg != undefined) {
				 var custom_message = methods._getCustomErrorMessage(jQuery(field), element_classes_array, alteredRule, options);
		 ...
		 _getCustomErrorMessage:function (field, classes, rule, options) {
			...
			var validityProp = /^custom\[.*\]jQuery/.test(rule) ? methods._validityProp["custom"] : methods._validityProp[rule];
		...
		_required: function(field, rules, i, options, condRequired) {
			switch (field.prop("type")) {
				...
				default:
					...
					var dv_placeholder = jQuery.trim( field.attr("data-validation-placeholder") );
		...
		*/
		_past: function(form, field, rules, i, options) {
			...
			var p=rules[i + 1];
			var fieldAlt = jQuery(form.find("input[name='" + p.replace(/^#+/, '') + "']"));
			var pdate;
			 ...
			 if (!options.isError && !methods._checkAjaxFieldStatus(field.attr("id"), options)) {
				 jQuery.ajax({
					 ...
					 success: function(json) {
						 ...
						 //var errorField = jQuery(jQuery("#" + errorFieldId)[0]);
						 var errorField = jQuery("#"+ errorFieldId).eq(0);
		...
		_buildPrompt: function(field, promptText, type, ajaxed, options) {
            ...
            if(requiredOverride !== undefined) {
                ...
                if(options.prettySelect) {
                    if(jQuery('#' + field.attr('id')).next().is('select')) {
                        var prettyOverrideClass = jQuery('#' + field.attr('id').substr(options.usePrefix.length).substring(options.useSuffix.length)).attr('data-required-class');
			...
			if(positionType === 'inline') {
				...
				if(typeof field.attr('data-prompt-target') !== 'undefined' && jQuery('#'+field.attr('data-prompt-target')).length > 0) {
					prompt.appendTo(jQuery('#'+field.attr('data-prompt-target')));
				} else {
					field.after(prompt);
				}
			} else {
		...
		*/
		_getPrompt: function(field) {
				var formId = jQuery(field).closest('form, .validationEngineContainer').attr('id');
			var className = methods._getClassName(field.attr("id")) + "formError";
				var match = jQuery("." + methods._escapeExpression(className) + '.parentForm' + formId)[0];
			if (match)
			return jQuery(match);
		},
		/**
		...
		*/
		 ...
		 */
		 _jqSelector: function(str){
			return str.replace(/([;&,\.\+\*\~':"\!\^#jQuery%@\[\]\(\)=>\|])/g, '\\jQuery1');
		},
		/**
	...
	// LEAK GLOBAL OPTIONS
	jQuery.validationEngine= {fieldIdCounter: 0,defaults:{
