<?php
global $obj_menu;
if(!empty($obj_menu->ad_integrations->adsense_id) && !empty($obj_menu->ad_integrations->adsense_slot)){
    $GLOBALS['google']['client'] = $obj_menu->ad_integrations->adsense_id;
    $GLOBALS['google']['https'] = read_global('HTTPS');
    $GLOBALS['google']['ip'] = read_global('REMOTE_ADDR');
    $GLOBALS['google']['markup'] = 'xhtml';
    $GLOBALS['google']['output'] = 'xhtml';
    $GLOBALS['google']['ref'] = read_global('HTTP_REFERER');
    $GLOBALS['google']['slotname'] = $obj_menu->ad_integrations->adsense_slot;
    ...
    if ($google_ad_handle) {
      while (!feof($google_ad_handle)) {
        $data = fread($google_ad_handle, 8192);
        
            echo '<div id="google_ads_top" style="background:#111; text-align:center; height:65px; margin:-15px -15px 0;">';
            echo $data ;
            echo '</div>';
